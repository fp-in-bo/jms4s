...
class JmsSession[F[_]: Sync: Logger: ContextShift] private[jms4s] (
  ...
  )(implicit C: Concurrent[F]): Resource[F, JmsMessageConsumer[F]] =
    for {
      consumer <- Resource.make(
                   Logger[F].info(
                     s"Opening MessageConsumer for Destination ${jmsDestination.wrapped}, Session: $wrapped..."
                   ) *>
                     blocker.delay(wrapped.createConsumer(jmsDestination.wrapped))
                 )(
                   c =>
                     Logger[F].info(
  ...
  def createProducer(jmsDestination: JmsDestination): Resource[F, JmsMessageProducer[F]] =
    for {
      producer <- Resource.make(
                   Logger[F].info(
                     ...
                     blocker.delay(wrapped.createProducer(jmsDestination.wrapped))
                 )(
                   c =>
                     Logger[F].info(
    ...
    } yield new JmsMessageProducer(producer, blocker)

  def createUnidentifiedProducer: Resource[F, JmsUnidentifiedMessageProducer[F]] =
    for {
      producer <- Resource.make(
